...
export type UiLanguage = "en" | "de" | "es" | "ru";
...
export type ReplyLanguage = "auto" | "en" | "de" | "es" | "ru";
...
const REPLY_VALUES: readonly ReplyLanguage[] = ["auto", "en", "de", "es", "ru"];

function isUiLanguage(v: unknown): v is UiLanguage {
  return v === "en" || v === "de" || v === "es" || v === "ru";
}
...
const RESOURCES: Record<UiLanguage, Record<string, unknown>> = {
  ...
  // ru.json currently covers the always-visible surfaces (nav, topbar,
  // sidebar, home, common, language settings, ...). Any key it doesn't have
  // falls through to English via resolve()'s fallback chain below, so a
  // partial translation degrades gracefully instead of showing "undefined".
...
const CHUNK_LOADERS: Record<LocaleChunk, Record<UiLanguage, () => Promise<unknown>>> = {
  marketplace: {
    ...
    // TODO(i18n/ru): not translated yet — serves English so the chunk still
    // resolves instead of leaving these views blank under "ru".
    ru: () => import("./locales/marketplace/en.json"),
  },
  local_models: {
    en: () => import("./locales/local_models/en.json"),
    de: () => import("./locales/local_models/de.json"),
    es: () => import("./locales/local_models/es.json"),
    ru: () => import("./locales/local_models/en.json"),
  },
  society: {
    en: () => import("./locales/society/en.json"),
    de: () => import("./locales/society/de.json"),
    es: () => import("./locales/society/es.json"),
    ru: () => import("./locales/society/en.json"),
  },
};

const EXTRA: Record<UiLanguage, Record<string, unknown>[]> = { en: [], de: [], es: [], ru: [] };
...
function readUi(): UiLanguage {
  try {
    ...
    if (raw === "en" || raw === "de" || raw === "es" || raw === "ru") return raw;
  } catch {
    /* SSR / private mode */
  }
  return "en";
